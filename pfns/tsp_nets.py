...
class Net(nn.Module):
    ...
    def forward(self, x, edge_index, edge_attr, batch, position=None, emb_net=None):
        return self.infer(
    ...
    @staticmethod
    def infer(x, edge_index, edge_attr, batch, emb_net, pooling_fn, position=None):
        ...
        # Get unique positions and batches (sorted for consistent ordering)
        unique_positions = torch.unique(position).sort()[0]
        unique_batches = torch.unique(batch).sort()[0]
        
        # Create tensor for graph embeddings with proper shape
